...
if __name__ == "__main__":
    DEV_URL = os.getenv("DEV_HOST")
    PROD_URL = os.getenv("PROD_HOST")
    LOCAL = "localhost"
    DEV_PATH = os.path.join(os.getenv("S3_WEAVIATE_PATH"), "dev")
    PROD_PATH = os.path.join(os.getenv("S3_WEAVIATE_PATH"), "prod")

    # 1️⃣ Backup DEV
    dev_backup = WeaviateBackupRestore(DEV_URL)
    backup_location, backup_id = dev_backup.create_backup(DEV_PATH)
    dev_backup.close()

    # 2️⃣ Restore to PROD (with automatic class deletion)
    prod_restore = WeaviateBackupRestore(PROD_URL)
